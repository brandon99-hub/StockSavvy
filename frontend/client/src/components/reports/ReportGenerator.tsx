...
const ReportGenerator = () => {
  ...
  const inventoryDataForReport = inventoryData?.products.map((product: Product) => ({
    ...product,
  ...
  const salesByDate = filteredSales.reduce((acc, sale) => {
    const dateStr = format(new Date(sale.sale_date), 'yyyy-MM-dd');
    if (!acc[dateStr]) {
  ...
  const handleExportCSV = () => {
    ...
    switch (reportType) {
      case 'inventory':
        data = inventoryDataForReport.map((item: any) => ({
          SKU: item.sku || '',
          Name: item.name,
        ...
        break;
      case 'sales':
        data = filteredSales.map((sale: Sale) => {
          const saleDate = new Date(sale.sale_date);
          return {
            'Sale ID': sale.id,
            Date: format(saleDate, 'yyyy-MM-dd HH:mm:ss'),
            'Total Amount': Number(sale.total_amount).toFixed(2),
            'Items Sold': sale.items_count || (sale.items_details?.length || 0),
    ...
    <div className="space-y-6">
      ...
      >
        <TabsList className={`grid w-full ${user?.role === 'staff' ? 'grid-cols-2' : 'grid-cols-3'}`}>
          ...
          {user?.role !== 'staff' && <TabsTrigger value="profit">Profit Report</TabsTrigger>}
        </TabsList>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {inventoryData?.products.map((item: Product, index: number) => (
                          <tr key={index}>
