...
export const exportInventoryToPDF = (products: Product[], categories: Record<number, string>) => {
  ...
  const tableData = products.map(product => [
    product.sku,
    product.name,
    categories[product.category_id || 0] || 'Uncategorized',
    (product.quantity || 0).toString(),
    `$${Number(product.buy_price || 0).toFixed(2)}`,
    `$${Number(product.sell_price || 0).toFixed(2)}`,
    (product as any).has_shop_inventory === false ? 'Pending' : (product.quantity || 0) < (product.min_stock_level || 0) ? 'Low Stock' : 'In Stock'
  ]);
...
export const exportSalesToPDF = (sales: Sale[], saleItems: Record<number, SaleItem[]>, products: Record<number, Product>) => {
  ...
  const salesData = sales.map(sale => [
    ...
    saleItems[sale.id]?.reduce((sum, item) => sum + (item.quantity || 0), 0).toString() || '0'
  ]);
